...
void processImage(const string& imagePath) {
    ...
    imwrite("hsv_image.jpg", hsvImage);
    // 应用均值滤波
    Mat meanBlurredImage;
    blur(image, meanBlurredImage, Size(15, 15));
    imwrite("mean_blurred_image.jpg", meanBlurredImage);
    // 应用高斯滤波
    Mat gaussianBlurredImage;
    GaussianBlur(image, gaussianBlurredImage, Size(15, 15), 0);
    imwrite("gaussian_blurred_image.jpg", gaussianBlurredImage);
    // 创建红色区域的掩码
    ...
    imwrite("red_contours.jpg", contourImage);
    ...
    imwrite("bounding_box_image.jpg", boundingBoxImage);
    // 计算轮廓面积
    for(size_t i = 0; i < contours.size(); i++) {
    ...
    imwrite("high_lighted_image.jpg", highLightedImage);
    // 绘制任意圆形、方形和文字
    ...
    imwrite("drawing_image.jpg", drawingImage);
